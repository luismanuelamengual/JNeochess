...
public class Board implements Disposable, Cloneable
{
    ...
    public static class Move implements Disposable, Comparable<Move>
    {
        ...
        public Move (String moveString) throws IllegalArgumentException
        {
            ...
            this.initialSquare = getSquare(moveString.substring(0, 2));
            this.endSquare = getSquare(moveString.substring(2, 4));
        }
        ...
        public String getSANString (Board board)
        {
            ...
            if ((movingPiece == WHITEKING && initialSquare == E1 && endSquare == G1) || (movingPiece == BLACKKING && initialSquare == E8 && endSquare == G8)) 
            {
                san = "O-O";
            }
            else if ((movingPiece == WHITEKING && initialSquare == E1 && endSquare == C1) || (movingPiece == BLACKKING && initialSquare == E8 && endSquare == C8))
            {
                san = "O-O-O";
            }
            else
            {
                ...
                if (movingFigure == PAWN) 
                {
                    if (board.getPiece(endSquare) != EMPTY || endSquare == board.getEnPassantSquare())
                    {
                        san += getFileChar(getSquareFile(initialSquare));
                        san += 'x';
                    }
                    san += getSquareString(endSquare);
                    byte endSquareRank = getSquareRank(endSquare);
                    if (endSquareRank == RANK_1 || endSquareRank == RANK_8)
                        san += "=Q";
                }
                else 
                {
                    san += getFigureChar(movingFigure);
                    if (board.getPiece(endSquare) != EMPTY)
                        san += 'x';                    
                    san += getSquareString(endSquare);
                }
        ...
        {
            return getSquareString(initialSquare) + getSquareString(endSquare);
        }
    }
}
